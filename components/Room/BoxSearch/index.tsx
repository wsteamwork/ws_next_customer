...
const BoxSearch: FC<IProps> = (props) => {
  ...
  const startDate = useSelector<ReducersList, string | null>((state) => state.searchFilter.startDate);
    ...
    <Paper className={classes.root} elevation={0}>
      <Grid container justify='space-between' alignItems='center'>
          ...
          <Collapse in={!collapseSearch}>
            <div>
              ...
              <Typography variant='subtitle2' color='textPrimary'>
                {t('room:lookingFor')} {checkIn}{checkOut}, {numberGuest} {t('room:guests')}, {numberRoom} {t('room:rooms')}
              </Typography>
            </div>
          </Collapse>
          <Collapse in={collapseSearch}>
        ...
        <Grid item xs={2} container justify='flex-end' alignItems='center'>
          <Button variant='outlined' className={classes.btnChange} size='large' onClick={handleCollapse}>
